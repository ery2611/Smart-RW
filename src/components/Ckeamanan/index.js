...
const rows = [
  ...
  createData(3, 'Yan Adhari', 'DITOLAK', '#EE1717'),
  createData(4, 'Dafta Syarif', 'DITOLAK', '#EE1717'),
];

function MyTable() {
  return (
    <Box sx={{  width: '100%', maxWidth: 500, margin: '0 auto', marginTop:'15px' }}>
      <Card sx={{ boxShadow:5}}>
      <CardHeader
          title={
              ...
              <TableBody>
                {rows.map((row) => (
                  <TableRow key={row.no}>
                    ...
                    <TableCell>
                      <Typography variant='caption' color={row.color}>{row.status}</Typography>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
